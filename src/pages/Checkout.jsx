...
function Checkout() {
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (result.valid) {
        ...
        setCouponMessage({ type: 'success', text: `✓ Coupon applied! You saved ₹${result.discountAmount}` });
        setTimeout(() => setCouponMessage(null), 3000);
      } else {
        setCouponDiscount(0);
        setCouponApplied(false);
        setCouponMessage({ type: 'error', text: data.error || result.message || 'Invalid coupon code' });
      }
    } catch (error) {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      // ✅ Sync address to backend database so it shows up in Profile page
      if (formData.saveAddress) {
        await fetch(`${API_URL}/api/users/addresses`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        <div className="max-w-2xl mx-auto px-4 py-12 flex-1 w-full">
          <div className="bg-white rounded-3xl shadow-xl border border-pink-100 p-8 text-center">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <Link to="/profile" className="bg-gradient-to-r from-pink-500 to-rose-500 text-white px-8 py-3 rounded-full font-semibold hover:shadow-lg shadow-pink-200 transition">
                View Orders 📦
              </Link>
