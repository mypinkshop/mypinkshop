...
const ProductCard = ({ product, addToCart, isInWishlist, addToWishlist, removeFromWishlist, user }) => {
  ...
  // ✅ Images एक बार parse करें
  const productImages = useMemo(() => parseImages(product.images), [product.images]);
  ...
  // ✅ Actual column names use करें
  const mrp = product.original_price || product.originalPrice || 0;
    ...
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-pink-50 relative flex flex-col">
      ...
      <Link to={`/product/${productId}`} className="block">
        <div className="relative h-48 sm:h-56 md:h-64 overflow-hidden bg-gradient-to-br from-pink-50 to-rose-50 flex items-center justify-center">
          ...
          {productImages[0] && !imgError ? (
            <img
              src={getOptimizedImage(productImages[0])}
...
function AccessoriesPage() {
  ...
  // ✅ Load products — actual schema use करें
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        // ✅ Actual column names: main_category, is_active (1/0)
        const accessoriesProducts = productsArray
          .filter(p => {
            const mainCat = (p.main_category || '').toLowerCase().trim();
            const isAccessories = mainCat === 'accessories' || mainCat === 'accessory';
            const isActive = p.is_active === 1 || p.is_active === true;
            return isAccessories && isActive;
          })
          .map(p => ({
            ...p,
            id: p._id || p.id,
  ...
  // ✅ Filter + sort — actual column names
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [products, searchTerm, selectedSubcategory, selectedBrand, minPrice, maxPrice, selectedDiscount, sortBy]);
  ...
  const subcategories = useMemo(() => {
    ...
    return [
      { id: 'all', name: 'All Subcategories', count: products.length },
      ...subs.map(s => ({
        id: s,
        name: s,
        count: products.filter(p => (p.sub_category || '').toLowerCase() === s.toLowerCase()).length,
      })),
    ];
  }, [products]);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Shop stunning accessories at MyPinkShop. Jewelry, bags, sunglasses, watches, and more. ✓ Free shipping ✓ COD ✓ Best prices." />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 text-center relative">
            ...
            <div className="relative z-10">
              ...
              <h1 className="text-2xl sm:text-4xl font-bold text-gray-900 mb-2">
                Complete Your <span className="text-pink-600">Look</span>
              </h1>
              <p className="text-gray-600 text-sm sm:text-base">
                {products.length}+ stunning accessories • Free shipping above ₹499
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {selectedSubcategory !== 'all' && (
                <span className="flex items-center gap-1.5 bg-pink-100 text-pink-700 text-xs font-semibold px-3 py-1.5 rounded-full">
                  {selectedSubcategory}
                  <button onClick={() => setSelectedSubcategory('all')} className="hover:text-pink-900">
                    ×
                  </button>
                </span>
              )}
              {selectedBrand !== 'all' && (
                <span className="flex items-center gap-1.5 bg-pink-100 text-pink-700 text-xs font-semibold px-3 py-1.5 rounded-full">
                  {selectedBrand}
                  <button onClick={() => setSelectedBrand('all')} className="hover:text-pink-900">
                    ×
                  </button>
                </span>
              )}
              {selectedDiscount > 0 && (
                <span className="flex items-center gap-1.5 bg-pink-100 text-pink-700 text-xs font-semibold px-3 py-1.5 rounded-full">
                  ...
                  <button onClick={() => setSelectedDiscount(0)} className="hover:text-pink-900">
                    ×
                  </button>
                </span>
              )}
              ...
              >
                {showFilters && (
                  <div className="flex justify-between items-center mb-4 lg:hidden">
                    ...
                    <button onClick={() => setShowFilters(false)} className="text-2xl text-gray-400">
                      ✕
                    </button>
                  </div>
                )}
              ...
              {filteredProducts.length === 0 ? (
                <div className="bg-white rounded-3xl p-12 sm:p-16 text-center border border-pink-100 shadow-sm">
                  ...
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    We're curating beautiful accessories for you. In the meantime, explore our makeup and skincare.
                  </p>
                  <div className="flex flex-wrap justify-center gap-3">
                    <Link
                      to="/makeup"
                      className="bg-gradient-to-r from-pink-500 to-rose-500 text-white px-6 py-3 rounded-full font-semibold hover:shadow-lg transition"
                    >
                      Shop Makeup 💄
                    </Link>
                    <Link
                      to="/skincare"
                      className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-6 py-3 rounded-full font-semibold hover:shadow-lg transition"
                    >
                      Shop Skincare ✨
                    </Link>
                  </div>
