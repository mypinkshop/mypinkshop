...
function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-rose-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
              {featuredProducts.map(product => (
                <ProductCard key={product.id} product={product} addToCart={addToCart} isInWishlist={isInWishlist} onWishlistToggle={() => {}} />
              ))}
            </div>
          </div>
        </section>
      ...
      {/* Premium Newsletter */}
      <section className="py-16 sm:py-20 bg-gradient-to-r from-pink-600 via-rose-600 to-pink-600">
        <div className="max-w-3xl mx-auto text-center px-4">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-2">Join the Pink Club</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" placeholder="Your email address" className="flex-1 px-5 py-3 rounded-full text-gray-900 focus:outline-none focus:ring-2 focus:ring-white" />
            <button className="bg-white text-pink-600 px-6 py-3 rounded-full font-semibold hover:shadow-lg transition transform hover:-translate-y-0.5">Subscribe</button>
          </div>
        </div>
      </section>
  ...
  <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-1 mb-2">
        <div className="flex text-yellow-400 text-sm">{'★'.repeat(Math.floor(product.rating || 4))}</div>
