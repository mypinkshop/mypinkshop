...
function ShippingInfo() {
  ...
  const handleNotify = () => {
    if (notifyEmail && notifyEmail.includes('@')) {
      alert(`Thanks! We'll notify you at ${notifyEmail} when international shipping launches.`);
      setNotifyEmail('');
    } else {
  ...
  const generateBreadcrumbSchema = () => ({
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Shipping Information", "item": "https://www.mypinkshop.com/shipping" }
    ]
  });
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="px-4 sm:px-6 py-3 text-left">Zone</th>
                        <th className="px-4 sm:px-6 py-3 text-left">Cities / Regions</th>
                        <th className="px-4 sm:px-6 py-3 text-left">Delivery Time</th>
                        <th className="px-4 sm:px-6 py-3 text-left">Standard Charge</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      {shippingZones.map((zone, idx) => (
                        <tr key={idx} className="hover:bg-pink-50/30 transition">
                          <td className="px-4 sm:px-6 py-3 font-medium text-gray-800">{zone.zone}</td>
                          <td className="px-4 sm:px-6 py-3 text-gray-600 text-sm">{zone.cities}</td>
                          <td className="px-4 sm:px-6 py-3 text-gray-600">{zone.days}</td>
                          <td className="px-4 sm:px-6 py-3 font-semibold text-pink-600">{zone.charge}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-pink-100 overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="px-4 sm:px-6 py-3 text-left">Country</th>
                        <th className="px-4 sm:px-6 py-3 text-left">Delivery Time</th>
                        <th className="px-4 sm:px-6 py-3 text-left">Estimated Charge</th>
                    ...
                    <tbody className="divide-y">
                      {internationalShipping.map((country, idx) => (
                        <tr key={idx} className="hover:bg-pink-50/30 transition">
                          <td className="px-4 sm:px-6 py-3 font-medium text-gray-800">{country.country}</td>
