...
function VendorSidebar({ activeTab }) {
  ...
  return (
    <aside className="fixed left-0 top-14 h-full w-64 bg-white border-r border-gray-200 shadow-sm overflow-y-auto z-40">
      <div className="p-4"><nav className="space-y-1">{menuItems.map(item => (<Link key={item.id} to={item.path} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${location.pathname === item.path ? 'bg-pink-50 text-pink-600 font-medium' : 'text-gray-600 hover:bg-gray-50'}`}><span>{item.icon}</span><span>{item.label}</span>{item.id === 'fba' && <span className="ml-auto text-xs bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded">Soon</span>}</Link>))}</nav></div>
    </aside>
  );
}
