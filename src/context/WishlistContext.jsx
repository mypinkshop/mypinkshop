  ...
  const fetchWishlist = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ✅ FIX: backend wraps as { success, data: [...] } — this was
        // always resetting to [] before because `json` itself is an
        // object, not an array.
        const backendWishlist = Array.isArray(json.data) ? json.data : (Array.isArray(json) ? json : []);
        setWishlist(backendWishlist);
        setWishlistCount(backendWishlist.length);
        // Also save to localStorage for consistency
        localStorage.setItem('guestWishlist', JSON.stringify(backendWishlist));
        console.log('🟢 CONTEXT - fetchWishlist: Fetched', backendWishlist.length, 'items from backend');
